...
# MongoDB connection
MONGODB_URI = os.environ.get("MONGODB_URI", "mongodb+srv://<username>:<password>@cluster0.mongodb.net/finance_tracker?retryWrites=true&w=majority")
...
async def create_transaction(transaction: Transaction, user: User = Depends(get_current_user)):
    ...
    transaction_dict["user_id"] = user.username
...
async def get_transactions(user: User = Depends(get_current_user)):
    transactions = []
    cursor = db.transactions.find({"user_id": user.username})
...
async def get_stats(user: User = Depends(get_current_user)):
    # Get total income
    income_pipeline = [
        {"$match": {"user_id": user.username, "transaction_type": "income"}},
        {"$group": {"_id": None, "total": {"$sum": "$amount"}}}
    ]
    income_cursor = db.transactions.aggregate(income_pipeline)
    income_result = await income_cursor.to_list(length=1)
    total_income = income_result[0]["total"] if income_result else 0
    
    # Get total expenses
    expense_pipeline = [
        {"$match": {"user_id": user.username, "transaction_type": "expense"}},
        {"$group": {"_id": None, "total": {"$sum": "$amount"}}}
    ]
    expense_cursor = db.transactions.aggregate(expense_pipeline)
...
async def get_settings(user: User = Depends(get_current_user)):
    settings = await db.settings.find_one({"user_id": user.username})
    if settings:
...
async def update_settings(settings: dict, user: User = Depends(get_current_user)):
    settings["user_id"] = user.username
